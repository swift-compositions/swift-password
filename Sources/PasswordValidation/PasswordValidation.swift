...
private func matches(pattern: String, in text: String) -> Bool {
    // reason: Regex.init(_:) is an untyped-throws stdlib callee (untyped-callee
    // boundary); an unparseable pattern is a programmer error here (all patterns
    // are local literal constants), so falling back to `false` is intentional.
